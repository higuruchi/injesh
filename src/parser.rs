...
If CMD is not specified, the default shell is used.";
    ...
    let app = App::new("injesh")
        ...
        .subcommand(App::new(INIT).about(init_about))
        .subcommand(
            // launchサブコマンド
            App::new(LAUNCH)
                .about(launch_about)
                ...
                .arg(Arg::new(OPT_ROOTFS).long(OPT_ROOTFS).takes_value(true))
                .arg(
                ...
                .arg(Arg::new(CMD).takes_value(true)),
        )
        .subcommand(
            // deleteサブコマンド
            App::new(DELETE)
            ...
            App::new(FILE)
                .subcommand(
                    ...
                    App::new(PULL)
                        ...
                        .arg(Arg::new("from").takes_value(true))
    ...
    match matches.subcommand() {
        ...
        Some((LIST, _)) => Ok(SubCommand::List(List::new()?)),
        Some((FILE, sub_m)) => {
            ...
            match sub_m.subcommand() {
                Some((PULL, sub_m)) => {
                    ...
                    Ok(SubCommand::File(FileSubCommand::Pull(File::new(
                ...
                _ => Err(Error::FileOperationNotFound)?,
            }
        }
